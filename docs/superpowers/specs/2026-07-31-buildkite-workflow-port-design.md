...
| 2 | self-hosted-cover-jobs.yml: LD_PRELOAD jemalloc under instrumentation | A coverage leg that finds/installs libjemalloc and LD_PRELOADs it over the std slice | HARD. ~5 builds to crack, all in agent-env quirks, none in the coverage logic: (1) resolving the lib path - the agent's /usr/lib is a symlink, so `find /usr/lib` without -L never descends (the lib was installed at /usr/lib/x86_64-linux-gnu/libjemalloc.so.2 the whole time); an `ls /usr/lib/*/` glob also didn't expand in the inline command shell. Fix: `find -L /`. (2) Doing install+lookup inside `JEM=$(fn)` under the prelude's `set -euo pipefail` swallowed the function tail - had to go flat inline. (3) A plain agent hit a transient `static.rust-lang.org` connection-reset during the prelude's Rust download (the leg needs no Rust, but the prelude installs the full toolchain unconditionally). Score reflects the debugging cost, not the final complexity. | 4 (provisional) |
| 3 | | | | |
